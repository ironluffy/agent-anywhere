#!/usr/bin/env python3
"""
Monitor tmux pane and stream output to hub
"""
import grpc
import sys
import os
            ...
            while self.running:
                ...
                if content and content != self.last_content:
                    ...
                    if self.last_content:
                        # Find the difference
                        lines_old = self.last_content.split('\n')
                        lines_new = content.split('\n')
                        
                        # Simple diff - send new lines at the end
                        if len(lines_new) > len(lines_old):
                            new_content = '\n'.join(lines_new[len(lines_old):])
                            yield agent_pb2.TerminalData(
                                session_id=self.terminal_session_id,
                                output=new_content,
                                timestamp=int(time.time() * 1000)
...
async def main():
    if len(sys.argv) < 2:
        print("Usage: monitor_tmux_pane.py <agent_id> [hub_url]")
        sys.exit(1)
